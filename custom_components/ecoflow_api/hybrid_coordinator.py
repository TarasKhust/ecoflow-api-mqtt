...
class EcoFlowHybridCoordinator(EcoFlowDataCoordinator):
    ...
    async def _async_handle_stop(self, _event: Any) -> None:
        ...
        Ensures MQTT client is properly disconnected before Home Assistant shuts down,
    ...
    async def _async_setup_mqtt(self) -> None:
        ...
        try:
            self._mqtt_client = EcoFlowMQTTClient(
                username=self.mqtt_username,  # type: ignore[arg-type]  # Guaranteed by guard clause
                password=self.mqtt_password,  # type: ignore[arg-type]  # Guaranteed by guard clause
            ...
            if connected:
                ...
                _LOGGER.info(
                    "✅ MQTT connected for device %s (hybrid: MQTT + REST every %ds)",
                    self.device_sn[-4:],
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            if _LOGGER.isEnabledFor(logging.DEBUG):
                ...
                _LOGGER.debug(
                    "🔄 [%s] REST UPDATE for %s (int=%ds, last=%.1fs, mqtt=%s)",
                    timestamp,
                    self.device_sn[-4:],
                    self.update_interval_seconds,
            ...
            if not self._logged_rest_success:
                ...
                _LOGGER.info(
                    "✅ REST API connected for device %s (%s mode, interval: %ds)",
                    self.device_sn[-4:],
                    mode,
                    self.update_interval_seconds,
                )
            ...
            if self._use_mqtt and self._mqtt_connected:
                ...
                if _LOGGER.isEnabledFor(logging.DEBUG):
                    timestamp = datetime.now().strftime("%H:%M:%S")
                    _LOGGER.debug(
                        "🔀 [%s] Merged data for %s: REST=%d + MQTT=%d = Total=%d",
                        timestamp,
                        self.device_sn[-4:],
                        len(rest_data),
                        len(self._mqtt_data),
